...
Free API for US-based users (alternative to global Binance).
...
class BinanceUSClient(BaseAPIClient):
    """
    Client for Binance.US Public API.
    
    ✅ FREE TIER: No API key required
    - Historical OHLC (candlestick) data
    - Market data for Binance.US-listed tokens
    - Rate limit: 1200 requests/minute
    
    ⚠️ US ONLY: For users in the United States
    ⚠️ LIMITATION: Fewer tokens than global Binance (~150 vs ~500)
    
    Documentation: https://docs.binance.us/
    """
    
    def __init__(self):
        """Initialize Binance.US client (no API key needed)."""
        super().__init__(api_key=None, base_url="https://api.binance.us/api/v3/")
        ...
        # Symbol mapping (Binance.US uses trading pairs like BTCUSDT)
        # NOTE: Binance.US has fewer tokens than global Binance
        self.symbol_to_pair = {
            "BTC": "BTCUSDT",
            "ETH": "ETHUSDT",
            "USDT": "USDTUSDT",
            ...
            "ETC": "ETCUSDT",
            "BCH": "BCHUSDT",
            # Note: TAO, TRX, and some others may not be available on Binance.US
        }
    
    def _get_trading_pair(self, symbol: str) -> str:
        """Convert symbol to Binance.US trading pair."""
    ...
    def get_klines(
        ...
        Reference: https://docs.binance.us/#kline-candlestick-data
        """
        pair = self._get_trading_pair(symbol)
        endpoint = "klines"
    ...
    def get_market_chart(self, symbol: str, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Binance.US klines fetch failed for {symbol}: {e}")
        ...
        return {
            "prices": prices,
            "market_caps": [],  # Binance.US doesn't provide historical market cap
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.info("Binance.US API connection test successful")
            return True
        except Exception as e:
            logger.error(f"Binance.US API connection test failed: {e}")
            return False
